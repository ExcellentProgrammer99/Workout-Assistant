...
public class Exercise_Curls : MonoBehaviour//
{
    ...
    List<string> mistakes = new List<string>();//list of mistakes made by the user
    ...
    void Start()
    {
        repetitions = SaveORLoadData.instance.activeSave.repetitionsEX4;
            totalseries = SaveORLoadData.instance.activeSave.seriesEX4;
    ...
    bool DoExercise()
    {
        ...
        if (inframe)//to prevent errors, the algorithm works only when the player is in the frame
        {
            ...
            if (part != 1)//first part of repetition->stay in initial position
            {
                ...
                {
                    part = 1;
                    if (timebetweenreps <= 1f)//if the time of part performance is less than 1sec->say that exercise is performed too fast; prevents injuries
            ...
            else
            {
                ...
                if (result && timebetweenreps >= 0.5f)//if the time of part performance is less than 0.5sec->say that exercise is performed too fast; prevents injuries
                {
                    part = 2;
                    count++;
    ...
    {
        if (sample.PositionsRelation("RIGHT_HIP", "RIGHT_WRIST", "y", 2f,  null) && sample.PositionsRelation("LEFT_HIP", "LEFT_WRIST", "y", 2f,  null))
            return true;
        return false;
    }
    ...
    {
        if (timer <= SaveORLoadData.instance.activeSave.highscore4)
        {
            SaveORLoadData.instance.activeSave.highscore4 = (int)timer;
            save.ApplyChanges();
            return true;
        }
        return false;
    ...
    bool GetfinalPosition()//final position->Bicep muscles contracted, the wrists are in place of shoulders;
                           //Algorithm predicts from proportions the location of shoulders and check if the user has wrists in their position
                           //if the prediction is wrong, checks if the learning model detected the shoulder position and compare whether user finished the repetition
                           //for better accuracy, model predicts position based on left and right sides of body
    {//from proportions half of distance between hip and the knee (which is the distance of one head) indicates the length between elbows and shoulders
        float LeftShoulderPrediction1= sample.SearchResult("LEFT_ELBOW", "y") + (sample.GetDistance(0,sample.SearchResult("LEFT_HIP","y"),0, sample.SearchResult("LEFT_KNEE", "y"))/2);
        float LeftShoulderPrediction2= sample.SearchResult("LEFT_ELBOW", "y") + (sample.GetDistance(0, sample.SearchResult("RIGHT_HIP", "y"), 0, sample.SearchResult("RIGHT_KNEE", "y")) / 2);

        float RightShoulderPrediction1 = sample.SearchResult("RIGHT_ELBOW", "y")+ (sample.GetDistance(0, sample.SearchResult("LEFT_HIP", "y"), 0, sample.SearchResult("LEFT_KNEE", "y"))/2);
        float RightShoulderPrediction2 = sample.SearchResult("RIGHT_ELBOW", "y") + (sample.GetDistance(0, sample.SearchResult("RIGHT_HIP", "y"), 0, sample.SearchResult("RIGHT_KNEE", "y")) / 2);

        if (sample.PositionsRelation("RIGHT_SHOULDER", "RIGHT_WRIST", "y", 3f, null)||sample.PositionsRelation("","","",3f, Mathf.Abs(sample.SearchResult("RIGHT_WRIST", "y") - RightShoulderPrediction1)))
            if (sample.PositionsRelation("LEFT_HIP", "LEFT_WRIST", "x", 5f, null))
                return true;
        if (sample.PositionsRelation("LEFT_SHOULDER", "LEFT_WRIST", "y", 3f, null)|| sample.PositionsRelation("", "", "", 3f, Mathf.Abs(sample.SearchResult("LEFT_WRIST", "y") - LeftShoulderPrediction1)))
            if(sample.PositionsRelation("RIGHT_HIP", "RIGHT_WRIST", "x", 5f, null))
                return true;
        if (sample.PositionsRelation("RIGHT_SHOULDER", "RIGHT_WRIST", "y", 3f, null) || sample.PositionsRelation("", "", "", 3f, Mathf.Abs(sample.SearchResult("RIGHT_WRIST", "y") - RightShoulderPrediction2)))
            if (sample.PositionsRelation("LEFT_HIP", "LEFT_WRIST", "x", 5f, null))
                return true;
        if (sample.PositionsRelation("LEFT_SHOULDER", "LEFT_WRIST", "y", 3f, null) || sample.PositionsRelation("", "", "", 3f, Mathf.Abs(sample.SearchResult("LEFT_WRIST", "y") - LeftShoulderPrediction2)))
            if (sample.PositionsRelation("RIGHT_HIP", "RIGHT_WRIST", "x", 5f, null))
                return true;
        return false;
    }
    ...
    {//Method used for explaining, showing and lisiting mistakes made during performance. The mistakes are later displayed on final screen of application.
        bool inframe = sample.PlayerIsInFrame();
        if (inframe)
        {
